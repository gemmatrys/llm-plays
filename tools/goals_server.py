"""Tiny read-only viewer for the run's CURRENT goal (first not [DONE]).

Standalone on purpose: separate process, separate port, so it survives
harness restarts and can be pointed at any run dir. Reads goals.md fresh
on every request — no caching, no watching.

    python tools/goals_server.py --run-dir runs/pokemon_red/limits-4 --port 8702
"""
